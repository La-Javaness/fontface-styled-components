...
const writeTransformedFonts = (dirname, filename, ttfBuffer, outputDir, forceRefresh) => {
...
const createFontFace = async (file, fontsOutputDir, styledOutputDir, fontsPublicDir, forceRefresh) => {
	const dirname = path.dirname(file.path)
	const filename = path.basename(file.path, '.ttf')

	await Promise.all([
		writeTransformedFonts(dirname, filename, file.content, fontsOutputDir, forceRefresh),
		writeCSSFontFaceTemplate(dirname, filename, file.content, styledOutputDir, fontsPublicDir),
	])
}
